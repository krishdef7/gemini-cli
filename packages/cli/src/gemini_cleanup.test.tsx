...
describe('gemini.tsx main function cleanup', () => {
  ...
  it('should register SessionEnd hook exactly once in non-interactive mode', async () => {
    ...
    const mockFireSessionEndEvent = vi.fn().mockResolvedValue(undefined);
    const mockHookSystem = {
      ...
      fireBeforeToolEvent: vi.fn().mockResolvedValue(undefined),
    ...
    // registerCleanup should be called exactly once for SessionEnd
    const sessionEndCalls = vi
      .mocked(registerCleanup)
      .mock.calls.filter((call) => {
        const fn = call[0].toString();
        return fn.includes('fireSessionEndEvent');
      });
